...
# mcp-atlassian, internal only (localhost:9000) -- never exposed directly.
/opt/venv/bin/mcp-atlassian --transport streamable-http --port 9000 -vv &
...
echo "Waiting for mcp-atlassian to bind :9000..."
i=0
until /opt/venv/bin/python3 -c "import socket; socket.create_connection(('127.0.0.1', 9000), timeout=1)" 2>/dev/null; do
    i=$((i + 1))
    if [ "$i" -ge 60 ]; then
        echo "mcp-atlassian did not bind :9000 within 60s -- starting nginx anyway" >&2
        break
    fi
    sleep 1
done
echo "mcp-atlassian ready after ${i}s, starting nginx"
