...
# logging-mcp, internal only (localhost:9000) -- never exposed directly.
...
echo "Waiting for logging-mcp to bind :9000..."
i=0
until /opt/venv/bin/python3 -c "import socket; socket.create_connection(('127.0.0.1', 9000), timeout=1)" 2>/dev/null; do
    i=$((i + 1))
    if [ "$i" -ge 60 ]; then
        echo "logging-mcp did not bind :9000 within 60s -- starting nginx anyway" >&2
        break
    fi
    sleep 1
done
echo "logging-mcp ready after ${i}s, starting nginx"
